...
switch( true ){
	case is_day():
		$page_title = esc_html__( 'Day: ', 'lolo' ) . get_the_date();
	break;
	case is_month():
		$page_title = esc_html__( 'Month: ', 'lolo' ) . get_the_date( esc_html_x( 'F Y', 'monthly archives date format', 'lolo' ) );
	break;
	case is_year():
		$page_title = esc_html__( 'Year: ', 'lolo' ) . get_the_date( esc_html_x( 'Y', 'yearly archives date format', 'lolo' ) );
	break;
	case is_search():
		$page_title = esc_html__( 'Search Results for: ', 'lolo' ) . get_search_query();
	break;
	case is_tag():
		$page_title = esc_html__( 'Tag: ', 'lolo' ) . single_tag_title( '', false );
	break;
	case is_category():
		$page_title = esc_html__( 'Category: ', 'lolo' ) . single_cat_title( '', false );
	break;
	case is_404():
		$page_title = esc_html__( 'OOPS! FILE NOT FOUND', 'lolo' );
	break;
	default:
		$page_title = esc_html__( 'Archives', 'lolo' );
	break;
}
...
<div class="container">
	...
	<div id="primary" class="content-area">
                ...
                <?php endif; ?>	
		<main id="main" class="site-main col-sm-9 columns-<?php echo esc_attr($columns); ?>"  style="<?php if( $page_column_class['left_sidebar'] || $page_column_class['right_sidebar']) { ?>width: 75%;<?php }else{ ?>width:100%;<?php } ?>">
